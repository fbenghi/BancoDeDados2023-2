...
-- 4.	Cadastre um novo Membro e cadastre para ele um empréstimo de 2 livros
INSERT INTO MEMBRO
(pnome, sobrenome, data_nascimento)
values
("Jubileu","da Silva","2001-01-01");

INSERT INTO EMPRESTIMO
(dataEmprestimo, idMembro)
VALUES
("2023-11-17", 21);
...
SELECT livro.idlivro, livro.nome FROM LivroEmprestimo
INNER JOIN Livro 
ON LivroEmprestimo.idLivro = Livro.idLivro
WHERE idEmprestimo = 41;

-- 5.	Descubra a data de nascimento do membro mais novo? 
SELECT MAX(data_nascimento) FROM Membro;

-- 6.	Qual o nome do membro mais velho?
SELECT * FROM MEMBRO
WHERE data_nascimento = (SELECT MIN(data_nascimento) FROM MEMBRO);

-- 7.	Qual é o membro com mais empréstimos?
SELECT 
    *
FROM
    MEMBRO
        INNER JOIN
    (SELECT 
        idMembro, COUNT(IDMembro) AS CONTAGEM
    FROM
        Emprestimo
    GROUP BY IDMembro
    ORDER BY CONTAGEM DESC
    LIMIT 1) AS maisEmprestimo ON maisEmprestimo.idMembro = MEMBRO.idmembro;

-- 8.	Quais empréstimos não foram devolvidos (data devolução = NULL)?
SELECT * FROM Emprestimo WHERE DataDevolucao IS NULL;

-- 9.	Quantos livros temos por categoria?
SELECT Categoria, COUNT(*) FROM Livro
GROUP By Categoria;

-- 10.	Quais categorias de livros têm mais do que 7 livros disponíveis?
SELECT Categoria, COUNT(*) 
FROM Livro
GROUP By Categoria
HAVING COUNT(*) > 7;

-- 11.	Qual o nome dos membros que ainda tem livros a devolver?
SELECT membro.pnome, membro.sobrenome FROM membro
inner join 
(
SELECT * FROM Emprestimo WHERE DataDevolucao IS NULL
) as devedores
ON devedores.idmembro = membro.idmembro;

SELECT distinct( membro.pnome), membro.sobrenome  FROM membro
inner join Emprestimo
ON Emprestimo.idmembro = membro.idmembro
WHERE Emprestimo.DataDevolucao IS NULL;

-- 12.	Quais os nomes dos livros que o membro chamado Pedro Alves emprestou?
SELECT * FROM MEMBRO
INNER JOIN EMPRESTIMO ON MEMBRO.idmembro = emprestimo.idmembro
INNER JOIN LivroEmprestimo ON LivroEmprestimo.idEmprestimo = emprestimo.idEmprestimo
INNER JOIN LIVRO ON Livro.IdLivro = LivroEmprestimo.idLivro
WHERE membro.pnome="Pedro" AND membro.sobrenome = "Alves";
